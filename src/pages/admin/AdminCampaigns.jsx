            ...
            <motion.div key={campaign.id}
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs px-2 py-0.5 bg-indigo-900/40 text-indigo-400 rounded-full flex-shrink-0">{campaign.category}</span>
                </div>
                ...
                <div className="h-1.5 bg-gray-800 rounded-full mt-2 overflow-hidden w-full max-w-xs">
                  <div className="h-full bg-gradient-to-r from-indigo-500 to-purple-500 rounded-full" style={{ width: `${progress}%` }} />
                </div>
              </div>
              <div className="flex items-center gap-2">
                <button className="p-2 text-gray-400 hover:text-indigo-400 hover:bg-indigo-900/20 rounded-xl transition-colors">
                  <Edit2 size={15} />
                </button>
                <button onClick={() => handleDelete(campaign.id)} className="p-2 text-gray-400 hover:text-red-400 hover:bg-red-900/20 rounded-xl transition-colors">
                  <Trash2 size={15} />
                </button>
              </div>
            </motion.div>
              ...
              <form onSubmit={handleCreate} className="space-y-4">
                {[
                  ...
                  <div key={f.key}>
                    ...
                    <input required={f.key !== 'image'} type={f.type} value={form[f.key]} onChange={e => setForm(p => ({ ...p, [f.key]: e.target.value }))}
                      className="w-full px-4 py-2.5 bg-gray-800 border border-gray-700 rounded-xl text-sm text-gray-200 placeholder-gray-500 focus:outline-none focus:border-indigo-500"
                ...
                <div>
                  ...
                  <textarea required rows={3} value={form.description} onChange={e => setForm(p => ({ ...p, description: e.target.value }))}
                    className="w-full px-4 py-2.5 bg-gray-800 border border-gray-700 rounded-xl text-sm text-gray-200 placeholder-gray-500 focus:outline-none focus:border-indigo-500 resize-none"
