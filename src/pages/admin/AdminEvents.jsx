          ...
          <motion.div key={event.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.1 }}
            ...
            <div className="relative h-32 overflow-hidden">
              ...
              <div className="absolute top-3 right-3 flex gap-2">
                <button className="p-1.5 bg-black/50 text-gray-300 hover:text-indigo-400 rounded-lg transition-colors">
                  <Edit2 size={13} />
                </button>
                <button onClick={() => { setEvents(prev => prev.filter(e => e.id !== event.id)); toast.success('Event removed'); }}
                  className="p-1.5 bg-black/50 text-gray-300 hover:text-red-400 rounded-lg transition-colors">
                  <Trash2 size={13} />
                </button>
              </div>
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }}
              ...
              <div className="flex items-center justify-between mb-6">
                ...
                <button onClick={() => setShowModal(false)} className="text-gray-400 hover:text-gray-600 dark:hover:text-white p-1"><X size={20} /></button>
              </div>
              <form onSubmit={handleCreate} className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <div>
                    <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">Entry Fee (₹)</label>
